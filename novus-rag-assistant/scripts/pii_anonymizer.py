"""
pii_anonymizer.py — Reversible PII anonymizer (Novus Bank RAG — Project A copy).
...
def redaction_audit_log(
    ...
    Fields: timestamp, trace_id, pii_types, query_hash, intent.
    """
    if log_path is None:
...
def main() -> None:
    ...
    if args.audit_test:
        ...
        logged = 0
        for intent, query in _audit_queries:
            anon = PiiAnonymizer()
            _ = anon.anonymize(query)
            if anon.has_pii():
                redaction_audit_log(query=query, anonymizer=anon, intent=intent, log_path=audit_path)
                logged += 1
            status = "LOGGED  " if anon.has_pii() else "skipped "
            print(f"  [{status}] {query[:70]:<70}  {anon.get_pii_types() if anon.has_pii() else []}")
        print(f"\nEntries: {logged}/20 queries had PII → {logged} log entries written")
        return
